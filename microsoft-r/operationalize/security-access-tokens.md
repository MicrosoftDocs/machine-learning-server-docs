...
|**Invalidity**|If bearer token older than 90 days, or<br><br>If `refresh_token` is expired, or<br><br>If user's password changed, or<br><br>If bearer token was revoked|If not used for 336 hours (14 days), or<br><br>When `access_token` expires, or<br><br>After `access_token` is used (replaced by another)<br> <br> <br>|
...
+ If Active Directory LDAP is enabled, then send a `POST /login HTTP/1.1` API request to retrieve the bearer token.
