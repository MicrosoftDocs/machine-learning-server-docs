...
+ Monitor or get the batch execution results with `results()`. This function takes `partialResults` as an argument that returns the already processed results of the batch execution even if it has not been fully completed. If omitted, then `partialResults = TRUE`.  If `partialResults = FALSE`, then it will only return the results if the execution has completed.  A batch result object is returned, which in our example is called `batchRes`.

+ Get the status of the batch execution with `STATUS`. 

+ Get the execution results for a given index row returned as an array with `execution`.   This function takes `index` as an argument that takes an index value for a given batch data record.
