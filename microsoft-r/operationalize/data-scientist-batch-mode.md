...
| `execution` |Get results for a given index row returned as an array  |[view](#results-fx)|
...
+ Monitor or get the batch execution results with `results(partialResults = TRUE)`, which takes `partialResults` as an argument. This argument returns the already processed results of the batch execution even if it has not been fully completed. If `partialResults = FALSE`, then it will only return the results if the execution has completed.  A batch result object is returned, which in our example is called `batchRes`.

+ Get the status of the batch execution with `STATUS`. 

+ Get the execution results for a given index row returned as an array using `execution(index)`.   This function takes the `index` value for a given batch data record as the argument.
