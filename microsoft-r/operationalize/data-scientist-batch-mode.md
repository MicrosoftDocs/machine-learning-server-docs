...
| ------------- |--------------------------------------------------------|
| `batch` |Define the data records to be batched and the thread count|
...
| Function      | Description                                            |
| ------------- |--------------------------------------------------------|
| `start` |Starts the execution of a batch scoring operation |
...
| `download` |Download one or all files from execution index|
...
Start the batch task with `start()`. R Server starts the batch execution and assigns an ID to the execution and returns the batch object. For example:
```R
TxBatch$start()
```
...
#### Get batch id
Get the batch task's identifier to reference during or after its execution using `id()`. R Server returns the ID for the named batch object. For example:
```R
TxBatch$id()
```

#### Cancel execution
Cancel the named batch execution using `cancel()`. R Server returns the batch object. 
...
+ Monitor or get the batch execution results with `results()`, which takes `partialResults` as an argument. This argument returns the already processed results of the batch execution even if it has not been fully completed. If `partialResults = FALSE`, then it will only return the results if the execution has completed.  A batch result object is returned, which in our example is called `batchRes`.

+ Get the status of the batch execution with `STATUS`. 

+ Get the execution results for a given index row returned as an array using `execution()`.   This function takes the `index` value for a given batch data record as the argument.

For example, the following code will monitor the results, return partial results every three seconds until the batch execution fails or completes, and then return results for a given index row returned as an array:
```R
batchRes <- NULL
while(TRUE) {
...
Retrieve a list of every file that was generated during the batch execution for a given data record, or index, with `listFiles()`. This can be made part of a loop to get the list of the files for every data record (see workflow example for a loop). For example:
...
Download the file(s) from a specific execution index using `download()`. By default, files are downloaded to the current working directory `getwd()` unless a different `dest = "<path>"` is specified.
...
For example, you can download a named file for the 5th index to a specified directory and then all files to the default working directory as follows:
