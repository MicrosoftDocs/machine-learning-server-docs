...
SQL Server provides an embedded machine learning feature that provides identical packages (RevoScaleR, revoscalepy, and so forth). The packages and language runtimes are hosted within the security and operational context of SQL Server, which fundamentally changes how you develop and deploy R and Python code. Despite having the same components, the installation and servicing of SQL Server Machine Learning Services and Microsoft Machine Learning Server is different. If you already have SQL Server, we strongly recommend that you look into the machine learning features in SQL Server. For more information, see [SQL Server Machine Learning Server](https://docs.microsoft.com/sql/advanced-analytics/home-advanced-analytics-r-machine-learning-sql-server).
