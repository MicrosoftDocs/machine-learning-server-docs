...
You can follow this guide from an R IDE of your choice.  On Windows, we recommend using the **R Productivity Environment** integrated development environment.  It is included with the client-side download of **Microsoft R Server** available for CTP3 customers. It has its own Getting Started Guide. More extensive general examples of using **RevoScaleR** can be found in the following guides included with **Microsoft R Server**:
