...
```
 ...
 ### `outFile`
 a character string specifying the output .xdf file, an  [RxXdfData](packagehelp/RxXdfData.md) object, a [RxOdbcData](packagehelp/RxOdbcData.md) data source, or a [RxTeradata](packagehelp/RxTeradata.md) data source.  If `NULL`, a data frame will be returned from `rxDataStep` unless `returnTransformObjects` is set to `TRUE`. Setting `outFile` to `NULL` and `returnTransformObjects=TRUE` allows chunkwise computations on the data without modifying the existing data or creating a new data set. `outFile` can also be a delimited [RxTextData](packagehelp/RxTextData.md) data source if using a native file system and not appending. 
 ...
 ### `transformFunc`
 variable transformation function. See [rxTransform](packagehelp/rxTransform.md) for details. 
  
  
    
 ### `transformVars`
 character vector of input data set variables needed for the transformation function. See [rxTransform](packagehelp/rxTransform.md) for details. 
...
*   `3`: rows processed and all timings are reported. 
 ...
 ### `makeRNodeNames`
 Determines if the names of the nodes should be normalized for use as R variables.   See also [rxMakeRNodeNames](packagehelp/rxMakeRNodeNames.md) for details on name mangeling. 
...
Use [rxImport](packagehelp/rxImport.md) instead of `rxImportToXdf`.
Use [rxDataStep](packagehelp/rxDataStep.md) instead of  `rxDataStepXdf`.
Use [rxDataStep](packagehelp/rxDataStep.md) instead of  `rxDataFrameToXdf`.
Use [rxDataStep](packagehelp/rxDataStep.md) instead of  `rxXdfToDataFrame`.
Use [rxSort](packagehelp/rxSortXdf.md) instead of  `rxSortXdf`.
Use [rxGetAvailableNodes](packagehelp/rxGetAvailableNodes.md) instead of  `rxGetNodes`.
...
[rxImport](packagehelp/rxImport.md),
[rxDataStep](packagehelp/rxDataStep.md),
