...
Here's an example you use to try out R Client in which we are predicting flight delays.
...
flight will be delayed by more than 15 minutes.  We approach this problem as a classification problem, 
...
1. Create a temporary directory to store the intermediate .xdf files.
...
### Limitations of R Client
Microsoft R Client allows you to work with production data locally  
using the full set of RevoScaleR functions; however, there are 
some constraints. On its own, the data to be processed must 
fit in local memory, and processing is capped at two threads 
for ScaleR functions. 

To benefit from disk scalability, performance and speed, you can 
push the compute context using rxSetComputeContext() to a 
production instance of Microsoft R Server such as 
SQL Server R Services and R Server for Hadoop. 

Learn about compatibility: 
https://msdn.microsoft.com/en-us/microsoft-r/r-client-compatibility
```
